...
class ImageViewer(object):
    ...
    * 'ESC' : quit
    ...
    def circle(self, x, y, radius, label=None):
        ...
        cv2.circle(image, center, int(radius + 0.5), self._color, self.thickness)
        if label is not None:
            cv2.putText(
                self.image, label, center, cv2.FONT_HERSHEY_PLAIN, 2, self.text_color, 2
            )
    ...
    def gaussian(self, mean, covariance, label=None):
        ...
        center = int(mean[0] + 0.5), int(mean[1] + 0.5)
        axes = int(vals[0] + 0.5), int(vals[1] + 0.5)
        angle = int(180.0 * np.arctan2(vecs[1, 0], vecs[0, 0]) / np.pi)
    ...
    def colored_points(self, points, colors=None, skip_index_check=False):
        ...
        if colors is None:
            colors = np.repeat(self._color, len(points)).reshape(3, len(points)).T
        indices = (points + 0.5).astype(np.int)
    ...
    def enable_videowriter(self, output_filename, fourcc_string="MJPG", fps=None):
        ...
        if fps is None:
            fps = int(1000.0 / self._update_ms)
