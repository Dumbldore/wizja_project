    ...
    def draw_trackers(self, tracks):
        ...
        for track in tracks:
            ...
            self.viewer.rectangle(
                *track.to_tlwh().astype(np.int), label=str(track.track_id) + track.label
            )

    def draw_detections(self, detections):
        self.viewer.thickness = 2
        for detection in detections:
            self.viewer.color = create_unique_color_uchar(randint(0, 255))
            self.viewer.rectangle(
                detection.tlwh.astype(np.int)[0],
                detection.tlwh.astype(np.int)[1],
                detection.tlwh.astype(np.int)[2],
                detection.tlwh.astype(np.int)[3],
                label=str(round(detection.confidence, 2)) + detection.label,
            )
