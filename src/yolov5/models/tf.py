...
from yolov5.utils.general import check_dataset, check_yaml, make_divisible
...
class tf_BottleneckCSP(keras.layers.Layer):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, w=None):
        ...
        self.m = keras.Sequential(
            [tf_Bottleneck(c_, c_, shortcut, g, e=1.0, w=w.m[j]) for j in range(n)]
        )
...
class tf_Detect(keras.layers.Layer):
    def __init__(self, nc=80, anchors=(), ch=(), w=None):  # detection layer
        ...
        self.m = [
            tf_Conv2d(x, self.no * self.na, 1, w=w.m[i]) for i, x in enumerate(ch)
        ]
        self.export = False  # onnx export
        self.training = True  # set to False after building model
        for i in range(self.nl):
            ny, nx = (
                opt.img_size[0] // self.stride[i],
                opt.img_size[1] // self.stride[i],
            )
    ...
    def call(self, inputs):
        ...
        for i in range(self.nl):
            ...
            x[i] = tf.transpose(
                tf.reshape(x[i], [-1, ny * nx, self.na, self.no]), [0, 2, 1, 3]
            )
            ...
            if not self.training:  # inference
                ...
                xy /= tf.constant(
                    [[opt.img_size[1], opt.img_size[0]]], dtype=tf.float32
                )
                wh /= tf.constant(
                    [[opt.img_size[1], opt.img_size[0]]], dtype=tf.float32
                )
...
class tf_Upsample(keras.layers.Layer):
    def __init__(self, size, scale_factor, mode, w=None):
        ...
        if opt.tf_raw_resize:
            ...
            self.upsample = lambda x: tf.raw_ops.ResizeNearestNeighbor(
                images=x, size=(x.shape[1] * 2, x.shape[2] * 2)
            )
        else:
...
class tf_Model:
    def __init__(
        self, cfg="yolov5s.yaml", ch=3, nc=None, model=None
        ...
        self.model, self.savelist = parse_model(
            deepcopy(self.yaml), ch=[ch], model=model
...
def agnostic_nms(x):
    ...
    selected_inds = tf.image.non_max_suppression(
        boxes,
        scores_inp,
        max_output_size=opt.topk_all,
        iou_threshold=opt.iou_thres,
        score_threshold=opt.score_thres,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--img-size", nargs="+", type=int, default=[320, 320], help="image size"
    )  # height, width
    parser.add_argument("--batch-size", type=int, default=1, help="batch size")
    parser.add_argument(
        "--dynamic-batch-size", action="store_true", help="dynamic batch size"
    )
    parser.add_argument(
        "--source",
        type=str,
        default="../data/coco128.yaml",
        help="dir of images or data.yaml file",
    )
    parser.add_argument(
        "--ncalib", type=int, default=100, help="number of calibration images"
    )
    parser.add_argument(
        "--tfl-int8",
        action="store_true",
        dest="tfl_int8",
        help="export TFLite int8 model",
    )
    parser.add_argument(
        "--tf-nms",
        action="store_true",
        dest="tf_nms",
        help="TF NMS (without TFLite export)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--topk-per-class", type=int, default=100, help="topk per class to keep in NMS"
    )
    parser.add_argument(
        "--topk-all", type=int, default=100, help="topk for all classes to keep in NMS"
    )
    parser.add_argument(
        "--iou-thres", type=float, default=0.5, help="IOU threshold for NMS"
    )
    parser.add_argument(
        "--score-thres", type=float, default=0.4, help="score threshold for NMS"
    )
    ...
    # Load PyTorch model
    model = attempt_load(
        opt.weights, map_location=torch.device("cpu"), inplace=True, fuse=False
    )
    ...
    try:
        print(
            "\nStarting TensorFlow saved_model export with TensorFlow %s..."
            % tf.__version__
        )
        ...
        inputs = keras.Input(
            shape=(*opt.img_size, 3),
            batch_size=None if opt.dynamic_batch_size else opt.batch_size,
        )
    ...
    try:
        print(
            "\nStarting TensorFlow GraphDef export with TensorFlow %s..."
            % tf.__version__
        )
